...
| `note` | `?string` | Optional | An optional note to associate with the checkout, as well as with any payments used to complete the checkout.<br>**Constraints**: *Maximum Length*: `250` | getNote(): ?string | setNote(?string note): void |
...
| `paymentType` | [`?string (TerminalCheckoutPaymentType)`](/doc/models/terminal-checkout-payment-type.md) | Optional | - | getPaymentType(): ?string | setPaymentType(?string paymentType): void |
