...
| `status` | `?string` | Optional | Filter results with the desired status of the `TerminalAction`<br>Options: PENDING, IN_PROGRESS, CANCELED, COMPLETED | getStatus(): ?string | setStatus(?string status): void |
