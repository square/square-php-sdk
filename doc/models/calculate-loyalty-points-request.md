...
|  --- | --- | --- | --- | --- | --- |
| `orderId` | `?string` | Optional | The [order](/doc/models/order.md) ID for which to calculate the points.<br>Specify this field if your application uses the Orders API to process orders.<br>Otherwise, specify the `transaction_amount`. | getOrderId(): ?string | setOrderId(?string orderId): void |
